...
public partial class XMLMetaReader : IMetaReader
{
    ...
    [GeneratedRegex(@"^(?<arg1>.*?);(?<arg2>.*)$")]
    private static partial Regex DoubleStringRegex();

    [GeneratedRegex(@"^(?<state>.*);(?<range>\d+([.]\d+)?);(?<time>\d+([.]\d+)?)$")]
    private static partial Regex WatchdogSetRegex();

    [GeneratedRegex(@"^(?<item>.*);(?<count>\d+)$")]
    private static partial Regex ItemCountRegex();

    [GeneratedRegex(@"^(?<spell>\d+);(?<time>\d+)$")]
    private static partial Regex TimeLeftOnSpellRegex();

    [GeneratedRegex(@"^(?<priority>\d+);(?<count>\d+);(?<distance>\d+([.]\d+)?)$")]
    private static partial Regex MonsterPriorityWithinDistanceRegex();

    [GeneratedRegex(@"^(?<name>.*);(?<count>\d+);(?<distance>\d+([.]\d+)?)$")]
    private static partial Regex MonsterCountWithinDistanceRegex();
    ...
    private async Task ReadAction(TextReader reader, MetaAction action)
    {
        ...
        switch (action)
        {
            ...
            case SetVTOptionMetaAction soa:
                m = DoubleStringRegex().Match(reader.ReadUntil('}'));
                if (!m.Success)
                    throw new MetaParserException("Invalid set VT option meta action");
                soa.Option = m.Groups["arg1"].Value;
                soa.Value = m.Groups["arg2"].Value;
                break;
            ...
            case GetVTOptionMetaAction goa:
                ...
                goa.Option = m.Groups["arg1"].Value;
                goa.Variable = m.Groups["arg2"].Value;
                break;
            ...
            case CallStateMetaAction csa:
                ...
                if (!m.Success)
                    throw new MetaParserException("Invalid get VT option meta action");
                csa.CallState = m.Groups["arg1"].Value;
                csa.ReturnState = m.Groups["arg2"].Value;
                break;

            case WatchdogSetMetaAction wsa:
                m = WatchdogSetRegex().Match(reader.ReadUntil('}'));
                if (!m.Success)
    ...
    private void ReadCondition(TextReader reader, Condition condition)
    {
        ...
        switch (condition)
        {
            ...
            case ItemCountCondition ic:
                m = ItemCountRegex().Match(reader.ReadUntil('}'));
                if (!m.Success)
            ...
            case ChatMessageCaptureCondition cmc:
                ...
                if (!m.Success)
                    throw new MetaParserException("Invalid condition string");
                cmc.Pattern = m.Groups["arg1"].Value;
                cmc.Color = m.Groups["arg2"].Value;
                break;

            case TimeLeftOnSpellGECondition tsc:
                m = TimeLeftOnSpellRegex().Match(reader.ReadUntil('}'));
                if (!m.Success)
                    throw new MetaParserException("Invalid condition string");
    ...
    {
        var m = MonsterPriorityWithinDistanceRegex().Match(conditionText);
        if (!m.Success)
    ...
    {
        var m = MonsterCountWithinDistanceRegex().Match(conditionText);
        if (!m.Success)
            throw new MetaParserException($"Invalid condition string: {conditionText}");
